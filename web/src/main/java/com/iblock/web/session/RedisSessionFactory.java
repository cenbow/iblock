...
public class RedisSessionFactory implements SessionFactory<RedisHttpSession> {
    ...
    public RedisSessionFactory(FilterConfig filterConfig) {
        ...
        // maxTotal
        String maxTotalVal = filterConfig.getInitParameter("maxTotal");
        if (maxTotalVal != null) {
            jedisPoolConfig.setMaxTotal(Integer.parseInt(maxTotalVal));
        }
    ...
    public RedisHttpSession getSession(HttpServletRequest request, HttpServletResponse response, boolean create) {
        ...
        try {
            ...
            if (sessionId == null) { // 新的会话
                session = new RedisHttpSession(jedisPool, interval, keyPrefix,
                        request.getSession().getServletContext());
            } else { // 关联已存在的会话
                session = new RedisHttpSession(sessionId, jedisPool, interval, keyPrefix,
                        request.getSession().getServletContext());
            }
    ...
    public void releaseSession(RedisHttpSession session, HttpServletRequest request) {
    }
